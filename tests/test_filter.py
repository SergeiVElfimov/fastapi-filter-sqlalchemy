...
@pytest.mark.parametrize(
    ...
    [
        ...
        [{"name__in": "Mr Praline,Mr Creosote,Gumbys,Knight"}, 3],
        [{"name__contains": "%Mr%"}, 2],
        [{"name__icontains": "%mr%"}, 2],
        [{"name__contains": "%colonel"}, 1],
        [{"name__contains": "Mr %"}, 2],
        [{"name__likein": "Mr %, %colonel"}, 2],
        [{"name__isnull": True}, 1],
        ...
        [{"search": "Mr"}, 2],
        [{"search": "mr"}, 2],
        [{"custom_filter": "San Francisco"}, 1],
        [{"custom_filter": "Mr Praline"}, 1],
    ],
)
...
@pytest.mark.usefixtures("users")
@pytest.mark.asyncio
async def test_filter_deprecation_contains_and_icontains(session, Address, User, UserFilter, filter_, expected_count):
    query = select(User).outerjoin(Address)
    with pytest.warns(DeprecationWarning, match="contains and icontains operators."):
        query = UserFilter(**filter_).filter(query)
    result = await session.execute(query)
    assert len(result.scalars().unique().all()) == expected_count
